...
>(
  ...
  Object.keys(keySchema).forEach((key) => {
    ...
    if (keyAttributes[key] === undefined)
      onError(`Key '${key}' not in keyAttributes`);
    if (schema.keyType === Table.PrimaryKeyType.Hash) {
      if (pk !== undefined)
        onError(
          `Key '${key}' invalid, ${name} already has partition key '${pk}'`
        );
      pk = key;
    }
    if (schema.keyType === Table.PrimaryKeyType.Range) {
      if (sk !== undefined)
        onError(`Key '${key}' invalid, ${name} already has sort key '${sk}'`);
      sk = key;
    }
  });
...
export function validateTable<KEY, ATTRIBUTES>(table: Table<KEY, ATTRIBUTES>) {
  ...
  table.globalIndexes?.forEach(({ name, keySchema }) => {
    ...
    if (ikeys === { pk, sk })
      onError(`${name} has same primary key '${ikeys}' as table`);
  });
  table.localIndexes?.forEach(({ name, keySchema }) => {
    ...
    const ikeys = validateKeySchema(keySchema, table.keyAttributes, name);
    if (ikeys.pk !== pk)
      onError(`${name} partition key '${ikeys.pk}' needs to be '${pk}'`);
