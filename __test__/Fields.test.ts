...
import { Field, FieldCompositeSlot } from '../src/Fields';
import { Model, ModelBase } from '../src/Model';
import { TableBase, Table } from '../src/Table';
...
const table = new Table<TableKey, TableKey>({
  name: 'MainTable',
  keyAttributes: {
    P: { type: 'S' },
    S: { type: 'S' },
...
describe('When FieldBase', () => {
  ...
  it('yup with coerce expect return value coerce', async () => {
    const field = Field.number().coerce().yup(yup.number().min(1).max(10));
    await expect(field.validate('5' as any)).resolves.toEqual(5);
  });

  it('joi expect validate invalid value to throw', async () => {
    const field = Field.number().joi(joi.number().min(1).max(2));
  ...
  it('joi with coerce expect return value coerce', async () => {
    const field = Field.number().joi(joi.number().min(1).max(10)).coerce();
    await expect(field.validate('5' as any)).resolves.toEqual(5);
  });

  it('regex expect validate invalid value to throw', async () => {
    const field = Field.string().regex(/^[A-Za-z][A-Za-z0-9]*$/);
  ...
  it('validator always throw expect to throw', async () => {
    const field = Field.string().validator((value: string) => {
  ...
  it('updateValidator with does not throw expect success', async () => {
    const field = Field.string().updateValidator((value: Model.ModelUpdateValue<string>) => {
      return new Promise<string | void>((resolve, reject) => {
        resolve('abc');
      });
    });
  ...
  it('updateValidator always throw expect to throw', async () => {
    const field = Field.string().updateValidator((value: any) => {
  ...
  it('constructor with alias expects alias to be set', () => {
    const field = Field.string('alias');
    expect(field._alias).toEqual('alias');
  });

  // toTablw
  it('toTable with hidden field expect not in table data', async () => {
    const field = Field.number().hidden();
    const tabelData: Table.AttributeValueMap = {};
  ...
  it('toTable with coerce expect coerce value', async () => {
    const field = Field.number().coerce();
    const tabelData: Table.AttributeValueMap = {};
  ...
  it('toTable with default function expects default return', async () => {
    const field = Field.string().default((name, tableData, modelData, model) => {
      return name + '-default';
    });
    const tabelData: Table.AttributeValueMap = {};
    await field.toTable('test', {}, tabelData, {} as ModelBase);
...
describe('When FieldMapT', () => {
  const field = Field.mapT<GroupModel, 'Groups'>('Groups', groupSchema);
  field.init('groups');
  it('expect size returns condition expression', () => {
    const exp = new ExpressionAttributes();
    expect(field.size()(exp)).toEqual('size(#n0)');
...
describe('When FieldDate', () => {
  ...
  it('toTable expect date as number', async () => {
    const data: Table.AttributeValueMap = {};
...
describe('When FieldComposite', () => {
  const field = Field.composite('G0S', 3);

  it('expect composite with delim returns correct type', () => {
    const delim = Field.composite('L0S', 2, '#');
...
describe('When FieldNamedComposite', () => {
  const field = Field.namedComposite('G0S', {
    city: 0,
    state: 1,
    country: 2,
  });
  ...
  it('expect constructed with slots', () => {
    const field1 = Field.namedComposite(
      'L0S',
      { dollar: 0, cents: 1 },
      {
