...
import { Condition, LogicalCondition, buildCondition } from "../src/Condition";
...
describe("Validate LogicalCondition", () => {
  ...
  it("add Conditions", () => {
    ...
    expect(logical.buildExpression(attribs)).toEqual(
      "#n0 <= :v0 AND #n1 >= :v1"
    );
  });
  ...
  it("add with 4 conditions", () => {
    const logical = LogicalCondition.and([cond1, cond2, cond3, cond4]);
    expect(logical.buildExpression(attribs)).toEqual(
      "#n0 <= :v0 AND #n1 >= :v1 AND #n2 = :v2 AND #n3 <> :v3"
    );
  });

  it("add with LogicalConditions", () => {
    ...
    expect(logical3.buildExpression(attribs)).toEqual(
      "(NOT #n0 <= :v0) AND (#n1 >= :v1 AND #n2 = :v2)"
    );
  });
  ...
  it("or with 4 conditions", () => {
    ...
    expect(logical.buildExpression(attribs)).toEqual(
      "#n0 <= :v0 OR #n1 >= :v1 OR #n2 = :v2 OR #n3 <> :v3"
    );
  });
...
describe("Validate buildCondition", () => {
  it("Equal", () => {
    expect(buildCondition("#path", ConditionOperator.Equal, [":v1"])).toEqual(
      "#path = :v1"
    );
  });
  it("NotEqual", () => {
    expect(
      buildCondition("#path", ConditionOperator.NotEqual, [":v1"])
    ).toEqual("#path <> :v1");
  });
  it("LessThen", () => {
    expect(
      buildCondition("#path", ConditionOperator.LessThen, [":v1"])
    ).toEqual("#path < :v1");
  });
  it("LessThenEqual", () => {
    expect(
      buildCondition("#path", ConditionOperator.LessThenEqual, [":v1"])
    ).toEqual("#path <= :v1");
  });
  it("GreaterThen", () => {
    expect(
      buildCondition("#path", ConditionOperator.GreaterThen, [":v1"])
    ).toEqual("#path > :v1");
  });
  it("GreaterThenEqual", () => {
    expect(
      buildCondition("#path", ConditionOperator.GreaterThenEqual, [":v1"])
    ).toEqual("#path >= :v1");
  });

  it("And with 2", () => {
    expect(
      buildCondition("cond1", ConditionOperator.And, ["cond2", "cond3"])
    ).toEqual("(cond1 AND cond2 AND cond3)");
  });
  it("And with 6", () => {
    expect(
      buildCondition("cond1", ConditionOperator.And, [
        "cond2",
        "cond3",
        "cond4",
        "cond5",
    ...
    ).toEqual("(cond1 AND cond2 AND cond3 AND cond4 AND cond5 AND cond6)");
  });

  it("Or with 2", () => {
    expect(
      buildCondition("cond1", ConditionOperator.Or, ["cond2", "cond3"])
    ).toEqual("(cond1 OR cond2 OR cond3)");
  });
  it("Or with 6", () => {
    expect(
      buildCondition("cond1", ConditionOperator.Or, [
        "cond2",
        "cond3",
        "cond4",
        "cond5",
    ...
    ).toEqual("(cond1 OR cond2 OR cond3 OR cond4 OR cond5 OR cond6)");
  });

  it("Not", () => {
    expect(buildCondition("cond1", ConditionOperator.Not)).toEqual(
      "(NOT cond1)"
    );
  });
  ...
  it("Between", () => {
    expect(
      buildCondition("#path", ConditionOperator.Between, [":v1", ":v2"])
    ).toEqual("#path BETWEEN :v1 AND :v2");
  });

  it("In with 1", () => {
    expect(buildCondition("#path", ConditionOperator.In, [":v1"])).toEqual(
      "#path IN (:v1)"
    );
  });
  ...
  it("In with 5", () => {
    expect(
      buildCondition("#path", ConditionOperator.In, [
        ":v1",
        ":v2",
        ":v3",
        ":v4",
    ...
    ).toEqual("#path IN (:v1, :v2, :v3, :v4, :v5)");
  });

  it("BeginsWith", () => {
    expect(
      buildCondition("#path", ConditionOperator.BeginsWith, [":v1"])
    ).toEqual("begins_with(#path, :v1)");
  });

  it("Contains", () => {
    expect(
      buildCondition("#path", ConditionOperator.Contains, [":v1"])
    ).toEqual("contains(#path, :v1)");
  });

  it("Type", () => {
    expect(buildCondition("#path", ConditionOperator.Type, [":v1"])).toEqual(
      "attribute_type(#path, :v1)"
    );
  });

  it("Exists", () => {
    expect(buildCondition("#path", ConditionOperator.Exists)).toEqual(
      "attribute_exists(#path)"
    );
  });

  it("NotExists", () => {
    expect(buildCondition("#path", ConditionOperator.NotExists)).toEqual(
      "attribute_not_exists(#path)"
    );
  });

  it("Size", () => {
    expect(buildCondition("#path", ConditionOperator.Size)).toEqual(
      "size(#path)"
    );
  });
});
